...
function getLogIcon(type) {
    switch (type) {
        case 'INFO': return { icon: 'mdi-information-outline', color: 'info' };
        case 'WARNING': return { icon: 'mdi-alert-outline', color: 'warning' };
        case 'ERROR': return { icon: 'mdi-alert-circle-outline', color: 'error' };
        case 'SUCCESS': return { icon: 'mdi-check-circle-outline', color: 'success' };
        default: return { icon: 'mdi-circle-outline', color: 'grey' };
    }
}
        ...
        <v-card-text class="flex-grow-1 pt-0">
            ...
            <template v-else>
                <div class="logs-scroll-container">
                    <v-table fixed-header density="comfortable">
                        ...
                        <tbody>
                            <tr v-for="log in logsStore.state.logs" :key="log.id" class="log-row">
                                <td>
                                    <v-icon :icon="getLogIcon(log.type).icon"
                                        :color="getLogIcon(log.type).color" size="18" />
                                </td>
                    ...
                    <div v-if="logsStore.hasMore" v-intersect="onIntersect" class="d-flex justify-center py-4">
                        <v-progress-circular v-if="logsStore.state.loadingMore" indeterminate color="primary"
                            size="28" width="3" />
                        <span v-else class="text-caption text-medium-emphasis">Carregando mais...</span>
                    </div>

                    <div v-if="!logsStore.hasMore && logsStore.state.logs.length > 0"
